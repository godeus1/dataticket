class ArticleAttachmentBlueprint < Blueprinter::Base
  identifier :id
  fields :filename, :content_type, :byte_size, :created_at

  association :user, blueprint: UserBlueprint, view: :summary

  # URL de download autenticada via endpoint da API (mesmo modelo dos tickets).
  field :download_url do |attachment|
    ...
    if S3Uploader.enabled?
      ...
      "/api/v1/articles/#{attachment.article_id}/attachments/#{attachment.id}/download"
    end
  end
end
