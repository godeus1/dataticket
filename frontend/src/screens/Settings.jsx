  ...
  return (
    <div style={{ maxWidth: 580 }}>
      ...
      <div className="card">
        ...
        <div className="form-row"><label className="label">E-mail remetente (SMTP_USER)</label><input className="input" type="email" value={form.emailSender} onChange={e => setForm(f => ({ ...f, emailSender: e.target.value }))} /></div>
        <div style={{ marginBottom: 14, padding: '10px 14px', background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: 8, fontSize: 12, color: '#92400e' }}>
          🔒 <strong>Senha SMTP:</strong> configure a variável de ambiente <code>SMTP_PASS</code> no painel do Vercel (Settings → Environment Variables). Não armazenamos a senha no código por segurança.
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px', gap: 10, marginBottom: 14 }}>
          <div><label className="label">Servidor SMTP (SMTP_HOST)</label><input className="input" value={form.smtpHost || ''} onChange={e => setForm(f => ({ ...f, smtpHost: e.target.value }))} placeholder="smtp.office365.com" /></div>
          <div><label className="label">Porta (SMTP_PORT)</label><input className="input" type="number" value={form.smtpPort || 587} onChange={e => setForm(f => ({ ...f, smtpPort: Number(e.target.value) }))} /></div>
        </div>
        <div className="form-row">
